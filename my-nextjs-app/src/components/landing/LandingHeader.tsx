...
export default function LandingHeader() {
  ...
  return (
    ...
    >
      <a
        href="#inhalt"
        className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-[100] focus:rounded-md focus:bg-[#141210] focus:px-4 focus:py-2.5 focus:text-sm focus:font-bold focus:text-[#FAF9F6]"
      >
        Zum Inhalt springen
      </a>
          ...
          <div className="flex flex-col gap-1">
            {navLinks.map((link) => (
              <a
                ...
                className="rounded-md px-3 py-2.5 text-[16px] font-medium text-[#26231E] hover:bg-[#F2EFE9] hover:text-[#A05F00]"
              >
                {link.label}
              </a>
            ))}
